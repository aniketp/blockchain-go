...
func (cli *CLI) Run() {
	...
	createBchainCmd := flag.NewFlagSet("createblockchain", flag.ExitOnError)

	// addBlockData := addBlockCmd.String("data", "", "Block data")
	blockchainData := createBchainCmd.String("address", "", "Owner name")

	/*
	 * blockchain printchain
	 * blockchain addblock -data "Send 1 BTC to Lavannya"
	 * blockchain createblockchain -address "Lavannya"
	 */
	switch os.Args[1] {
	// case "addblock":
	...
	case "createblockchain":
		err := createBchainCmd.Parse(os.Args[2:])
		if err != nil {
			log.Fatal("Error in parsing arguments")
		}
	default:
	...
	if createBchainCmd.Parsed() {
		if *blockchainData == "" {
			/* No additional arguments were supplied */
			createBchainCmd.Usage()
			os.Exit(1)
		}
		cli.createBlockChain(*blockchainData)
	}
